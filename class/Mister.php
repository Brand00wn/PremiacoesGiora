...
class Mister extends DBConnection
{
	...
	public function save_product_auto_cota() {
		...
		$quantidade_auto_cota = $_POST['quantidade_auto_cota'] ?? '0';
		...
		$stmt = $this->conn->prepare('UPDATE `product_list` SET `status_auto_cota` = ?, `tipo_auto_cota` = ?, `quantidade_auto_cota` = ? WHERE id = ?');
		if (!$stmt) {
			return json_encode([
				"status" => "failed",
		...
		$stmt->bind_param('ssii', $status_auto_cota, $tipo_auto_cota, $quantidade_auto_cota, $product_id);
    ...
    public function get_product_auto_cota(){
        ...
        $qry = $this->conn->query('SELECT `status_auto_cota`, `tipo_auto_cota`,`quantidade_auto_cota` FROM `product_list` WHERE id = \'' . $this->conn->real_escape_string($product_id) . '\'');
        if ($qry->num_rows > 0) {
            $product = $qry->fetch_assoc();
            $resp["status"] = "success";
            $resp["status_auto_cota"] = $product['status_auto_cota'];
            $resp["tipo_auto_cota"] = $product['tipo_auto_cota'];
			$resp["quantidade_auto_cota"] = $product['quantidade_auto_cota'];
        } else {
            $resp["status"] = "failed";
